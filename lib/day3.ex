defmodule Day3 do
  ...
  def main() do
    {_, wire1} = record_moves([0, 0], @wire1)
    {_, wire2} = record_moves([0, 0], @wire2)

    closest_crossing([0, 0], wire1, wire2)
  end

  def main2() do
    {_, wire1} = record_moves(@wire1)
    {_, wire2} = record_moves(@wire2)

    shortest_crossing(wire1, wire2)
  end

  def record_moves(moves) do
    record_moves([0, 0], moves)
  end

  def record_moves(start_pos, moves) when is_bitstring(moves) do
    record_moves(start_pos, String.split(moves, ","))
  end

  def record_moves(start_pos, moves) do
    {new_pos, points} =
      Enum.reduce(moves, {start_pos, []}, fn move, {pos, points} ->
        {new_pos, new_points} = record_move(pos, move)
  ...
  def record_move(move) do
    record_move([0, 0], move)
  end

  def record_move(start_pos = [x, y], move) do
    new_pos = Day3.move(start_pos, move)
    {dir, len} = parse_move(move)
  ...
  def parse_move(move) do
    [_, dir, length] = Regex.run(@move_pattern, move)
    {@directions[dir], String.to_integer(length)}
  end

  def grid_distance([x1, y1], [x2, y2]) do
    abs(x1 - x2) + abs(y1 - y2)
  end

  def closest_crossing(moves1, moves2) do
    closest_crossing([0, 0], moves1, moves2)
  end

  def closest_crossing(pos, moves1, moves2) do
    MapSet.intersection(MapSet.new(moves1), MapSet.new(moves2))
    |> MapSet.delete(pos)
    |> Enum.min_by(&grid_distance(pos, &1))
    |> grid_distance(pos)
  end

  def shortest_crossing(moves1, moves2) do
    moveSteps1 = Enum.with_index(moves1, 1)
    moveSteps2 = Enum.with_index(moves2, 1)

    MapSet.intersection(MapSet.new(moves1), MapSet.new(moves2))
    |> Enum.map(fn c ->
      {_, steps} = Enum.find(moveSteps1, fn {pos, _} -> c == pos end)
      {_, steps2} = Enum.find(moveSteps2, fn {pos, _} -> c == pos end)
      steps + steps2
    end)
    |> Enum.min
  end
